...
from KwentasApp.models import Entry  # Adjust based on your actual model name

def get_processed_project_data():
    # Ensure you retrieve the data correctly
    projects = Entry.objects.all()  # Adjust as necessary for your project query
    project_data = []
...
def indexPage(request):
    ...
    filtered_entries = sorted(all_entries, key=lambda x: x.get('utilization_rate', 0), reverse=True)
    
    # Debugging: Check the content of filtered_entries
    print(f"Filtered Entries: {filtered_entries}")

    return render(request, 'KwentasApp/index.html', {
